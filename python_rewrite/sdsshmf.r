...
for (k in 1:1000){
  ...
  tryz[k]=k/1000.0}
for (i in 1:length(sdssgals$dmax)){sdssgals$zmax[i]=tryz[which.min(abs(sdssgals$dmax[i]-try))]}
...
# meancounts
for(i in 1:length(sdsshmf$mids)){meancounts[i]=mean(mockcounts[,i])}
for(i in 1:length(sdsshmf$mids)){mediancounts[i]=quantile(mockcounts[,i],0.5)}
for(i in 1:length(sdsshmf$mids)){upcounts[i]=quantile(mockcounts[,i],0.84)}
for(i in 1:length(sdsshmf$mids)){docounts[i]=quantile(mockcounts[,i],0.16)}
...
# MC err
#
for(i in 1:length(sdsshmf$mids)){mcerr[i]=quantile((meancounts[i]-mockcounts[,i])^2,0.66)^0.5/(meancounts[i])}
#
# Poisson Error
#
