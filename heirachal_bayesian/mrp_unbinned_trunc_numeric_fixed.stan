  ...
  functions {
    ...
    // log of unnormalized MRP over log10 M (plain helper; NOT a _lp/_lpdf)
    real mrp_log10_unnorm_log(real x, real mstar, real alpha, real beta) {
      real t = pow(10.0, beta * (x - mstar));  // u = 10^{beta(x-m*)}
      return log(beta) + log(ln10()) + ln10() * (alpha + 1.0) * (x - mstar) - t;
    }

    // Integrand for integrate_1d must be: f(x, xc, theta, x_r, x_i)
    real mrp_unnorm(real x, real xc, real[] theta, real[] x_r, int[] x_i) {
      real mstar = theta[1];
      real alpha = theta[2];
      real beta  = theta[3];
      return exp( mrp_log10_unnorm_log(x, mstar, alpha, beta) );
    }

    // log normalization Z(theta; x_min) = ∫_{x_min}^{XUP} f(x|θ) dx
    real log_Z_numeric(real mstar, real alpha, real beta, real x_min) {
      real theta[3];
      theta[1] = mstar; theta[2] = alpha; theta[3] = beta;
      real x_r[0];
      int x_i[0];
      real XUP = 18.5; // far into the tail; 10^18.5 Msun >> any halo
  ...
  data {
    ...
    vector<lower=0>[N] w_shape;     // per-object shape weights (start at 1)
  ...
  model {
    // Priors (centered on GAMA regime)
    ...
    // Likelihood: shape-only + measurement (Eddington bias generative)
    for (n in 1:N) {
      target += w_shape[n] * ( mrp_log10_unnorm_log(x_true[n], mstar, alpha, beta) - logZ );
      x_obs[n] ~ normal(x_true[n], sigma_x[n]);
    }
  }
